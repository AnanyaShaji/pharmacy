         ...
         for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach($Reader as $Row){
            	...
            	if(isset($Row[1])){
                    ...
                    $Rack = mysqli_real_escape_string($con,$Row[1]);
            	...
            	if(isset($Row[13])){
            	  ...
            	  if (!empty($medicine_name) || !empty($category) || !empty($Rack) || !empty($quantity) || !empty($used_quantity) || !empty($remain_quantity) || !empty($act_remain_quantity) || !empty($register_date) ||  !empty($expire_date) ||  !empty($company) || !empty($sell_type) || !empty($actual_price) || !empty($selling_price) || !empty($profit_price) || !empty($status)) {

                    $query = "INSERT INTO stock(medicine_name, category, Rack, quantity,used_quantity,remain_quantity,act_remain_quantity, register_date, expire_date, company, sell_type , actual_price,selling_price, profit_price,status)  values('".$medicine_name."','".$category."','".$Rack."','".$quantity."','".$used_quantity."','".$remain_quantity."','".$remain_quantity."','".$register_date."','".$expire_date."','".$company."','".$sell_type."','".$actual_price."','".$sell_type."','".$profit_price."','".$status."')";
