...
class myPDF extends FPDF{
  ...
  function header(){
    ...
    $this->Cell(276,10,'MedShop Invoice',0,0,'C');
    $this->Ln(20);
 ...
 function viewTable(){
  ...
  include "dbcon.php";
     ...
     $invoice_number = $_POST['invoice_number'];
       ...
       $select_sql = "SELECT * FROM cart where invoice_number = '$invoice_number'";
    ...
    $select_sql = "SELECT sum(amount) from cart where invoice_number = '$invoice_number'";
...
$date1 = date("YMd");
  ...
  if(isset($_POST['submit'])){
    ...
    // Corrected the order of parameters for implode
    ...
    $qty_type = $qty;  // You might want to reconsider this line. It seems redundant.
    
    $filename = "i-" . $invoice_number . ".pdf";
    $dir = "";  // If you're defining an empty directory path, ensure you have plans to set it later.
  }
 ...
 $select_cart = "SELECT * From cart where invoice_number = '$invoice_number'";

 $select_cart_query = mysqli_query($con,$select_cart);

  $row = mysqli_fetch_array($select_cart_query);
  ...
  $remain_quantity = $row['remain_quantity'];
	...
	$select_sql = "SELECT invoice_number ,sum(amount) ,sum(profit_amount) FROM cart where invoice_number = '$invoice_number'";
	$select_query  = mysqli_query($con,$select_sql);

	while ($row = mysqli_fetch_array($select_query)) {
     $cart_invoice=$row['invoice_number'];
